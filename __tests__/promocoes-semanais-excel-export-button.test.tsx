...
describe('PromocoesSemanaisExcelExportButton', () => {
  const mockGraphData = [
    { dataEnvio: '01/01', '1': 10, '2': 5 },
    { dataEnvio: '02/01', '1': 15, '2': 8 }
  ]
  const mockLojas = ['1', '2']
  ...
  it('should show loading state when exporting', async () => {
    mockExcelExportService.exportPromocoesSemanaisToExcel.mockImplementation(() => {
  ...
  it('should call export service when clicked', async () => {
    mockExcelExportService.exportPromocoesSemanaisToExcel.mockResolvedValue()
    ...
    await waitFor(() => {
      expect(mockExcelExportService.exportPromocoesSemanaisToExcel).toHaveBeenCalledWith(
        mockGraphData, 
        mockLojas,
        "Test Title"
      )
  ...
  it('should show success toast when export succeeds with data', async () => {
    ...
    await waitFor(() => {
      expect(mockToast.success).toHaveBeenCalledWith('Dados de promoções da última semana exportados com sucesso!')
    })
  })
  ...
  it('should show error toast when export fails', async () => {
    ...
    mockExcelExportService.exportPromocoesSemanaisToExcel.mockRejectedValue(error)

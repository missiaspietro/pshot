...
export interface PromocoesAnuaisExcelExportButtonProps {
...
export function PromocoesAnuaisExcelExportButton({
  graphData,
  lojas,
  title,
  isLoading = false,
  disabled = false,
  className
}: PromocoesAnuaisExcelExportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      excelExportService.exportPromocoesAnuaisToExcel(graphData, lojas, title)
      
      if (!graphData || graphData.length === 0) {
        toast.success('Arquivo Excel exportado! (Sem dados no gráfico)')
      } else {
        toast.success('Dados de promoções dos últimos 6 meses exportados com sucesso!')
      }
    } catch (error) {

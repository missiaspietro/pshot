...
export interface PromocoesSemanaisExcelExportButtonProps {
  graphData: any[] // Dados do gráfico no formato { dataEnvio: string, [loja]: number }
...
export function PromocoesSemanaisExcelExportButton({
  graphData,
  lojas,
  title,
  isLoading = false,
  disabled = false,
  className
}: PromocoesSemanaisExcelExportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      excelExportService.exportPromocoesSemanaisToExcel(graphData, lojas, title)
      
      if (!graphData || graphData.length === 0) {
        toast.success('Arquivo Excel exportado! (Sem dados no gráfico)')
      } else {
        toast.success('Dados de promoções da última semana exportados com sucesso!')
      }
    } catch (error) {

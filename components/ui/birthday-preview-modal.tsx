...
import { FileText, Download, Loader2, AlertCircle, RefreshCw, Wifi, WifiOff, ChevronLeft, ChevronRight, Cake } from "lucide-react"
import { normalizeText } from "@/lib/text-utils"

interface BirthdayPreviewModalProps {
  isOpen: boolean
  onClose: () => void
...
export function BirthdayPreviewModal({
  ...
  selectedStore,
  fieldLabels
}: BirthdayPreviewModalProps) {
  const [allData, setAllData] = useState<BirthdayData[]>([])
  ...
  const fetchData = async () => {
    console.log('🔄 Iniciando busca de dados de aniversários...')
    setIsLoading(true)
    setError(null)
    
    try {
      console.log('📤 Enviando requisição para API de aniversários...')
      ...
      console.log('✅ Dados de aniversários recebidos:', result)

      // USAR DADOS EXATAMENTE COMO VÊM DO BANCO - SEM PROCESSAMENTO
      const rawData = result.data || []
      
      console.log('🔍 Dados brutos de aniversários (sem modificação):', rawData.length, 'registros')
      console.log('🔍 Primeiro item bruto:', rawData[0])
      
      // Verificar se há nomes vazios nos dados originais
      const nomesVaziosOriginais = rawData.filter((item: any) => !item.cliente || item.cliente === '' || item.cliente === null)
      if (nomesVaziosOriginais.length > 0) {
        ...
        nomesVaziosOriginais.slice(0, 3).forEach((item: any, index: number) => {
          console.warn(`   ${index + 1}. Cliente: "${item.cliente}", Status: "${item.observações}", WhatsApp: "${item.whatsApp}"`)
        })
      }
      ...
      console.error('💥 Erro ao buscar dados de aniversários:', error)
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      console.log('🔄 Iniciando geração de relatório de aniversários PDF...')
      console.log('📋 Campos selecionados:', selectedFields)
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Erro ao gerar relatório de aniversários')
      }
      ...
      if (contentType?.includes('application/pdf')) {
        ...
        console.log('✅ PDF de aniversários gerado com sucesso')
        ...
        console.log('⚠️ Fallback: HTML de aniversários gerado (Puppeteer falhou)')
      ...
      console.error('💥 Erro ao gerar relatório de aniversários:', error)
      
      // Mostrar erro detalhado para o usuário
      const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido'
      alert(`Erro ao gerar relatório de aniversários: ${errorMessage}`)
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col" aria-describedby="birthday-preview-description">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Cake className="h-5 w-5 text-pink-600" />
            Preview do Relatório de Aniversários
          </DialogTitle>
          <div id="birthday-preview-description" className="text-sm text-gray-500">
          ...
          {isLoading ? (
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center text-gray-500">
                ...
                <div>Nenhum dado de aniversários encontrado para a sua empresa</div>
              </div>
            </div>
          ) : (

...
// ✅ MODAL RECONECTADO AO BANCO DE DADOS
// Este modal está conectado à tabela "Relatorio Envio de Promoções" 
// Filtra dados pela empresa do usuário logado (campo "Rede")
...
import { Download, Loader2, AlertCircle, RefreshCw, WifiOff, ChevronLeft, ChevronRight, Percent } from "lucide-react"
import { normalizeText } from "@/lib/text-utils"

interface PromotionsPreviewModalProps {
  isOpen: boolean
  onClose: () => void
...
export function PromotionsPreviewModal({
  isOpen,
  onClose,
  selectedFields,
  startDate,
  ...
  selectedStore,
  fieldLabels
}: PromotionsPreviewModalProps) {
  const [allData, setAllData] = useState<PromotionsData[]>([])
  ...
  const fetchData = async () => {
    console.log('🔄 Iniciando busca de dados de promoções...')
    setIsLoading(true)
    setError(null)
    
    try {
      console.log('📤 Enviando requisição para API de promoções...')
      ...
      console.log('✅ Dados de promoções recebidos:', result)

      // USAR DADOS EXATAMENTE COMO VÊM DO BANCO - SEM PROCESSAMENTO
      const rawData = result.data || []
      
      console.log('🔍 Dados brutos de promoções (sem modificação):', rawData.length, 'registros')
      console.log('🔍 Primeiro item bruto:', rawData[0])
      
      // Verificar se há clientes vazios nos dados originais
      const clientesVaziosOriginais = rawData.filter((item: any) => !item.Cliente || item.Cliente === '' || item.Cliente === null)
      if (clientesVaziosOriginais.length > 0) {
        console.warn('⚠️ DADOS ORIGINAIS: Encontrados', clientesVaziosOriginais.length, 'registros sem cliente no banco de dados')
        clientesVaziosOriginais.slice(0, 3).forEach((item: any, index: number) => {
          console.warn(`   ${index + 1}. Cliente: "${item.Cliente}", Enviado: "${item.Enviado}", WhatsApp: "${item.Whatsapp}"`)
        })
      }
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      console.log('🔄 Iniciando geração de relatório de promoções PDF...')
      console.log('📋 Campos selecionados:', selectedFields)
      console.log('📅 Período:', { startDate, endDate })

      const response = await fetch('/api/reports/promotions/pdf', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Erro ao gerar relatório de promoções')
      }
      ...
      if (contentType?.includes('application/pdf')) {
        ...
        console.log('⚠️ Fallback: HTML de promoções gerado (Puppeteer falhou)')
      ...
      alert(`Erro ao gerar relatório de promoções: ${errorMessage}`)
    } finally {
  ...
  const formatCellValue = (value: any) => {
    ...
    if (typeof value === 'string' && value.includes('T')) {
      try {
        ...
        // Se não conseguir formatar como data, normalizar o texto
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col" aria-describedby="promotions-preview-description">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Percent className="h-5 w-5 text-yellow-600" />
            Preview do Relatório de Promoções
          </DialogTitle>
          <div id="promotions-preview-description" className="text-sm text-gray-500">
          ...
          {isLoading ? (
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center text-gray-500">
                ...
                <div>Nenhum dado de promoções encontrado para a sua empresa</div>
              </div>
            </div>
          ) : (
                ...
                <TableBody>
                  {currentData.map((row, index) => (
                    <TableRow key={startIndex + index}>
                      {selectedFields.map((field) => {
                        // Usar os campos diretamente da tabela "Relatorio Envio de Promoções"

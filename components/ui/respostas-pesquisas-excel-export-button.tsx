...
export interface RespostasPesquisasExcelExportButtonProps {
  graphData: any[] // Dados do gráfico no formato { loja: string, Ótimo: number, Bom: number, Regular: number, Péssimo: number }
  title: string
  isLoading?: boolean
  disabled?: boolean
  className?: string
...
export function RespostasPesquisasExcelExportButton({
  graphData,
  title,
  isLoading = false,
  disabled = false,
  className
}: RespostasPesquisasExcelExportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      excelExportService.exportRespostasPesquisasToExcel(graphData, title)
      
      if (!graphData || graphData.length === 0) {
        toast.success('Arquivo Excel exportado! (Sem dados no gráfico)')
      } else {
        toast.success('Dados de respostas de pesquisas exportados com sucesso!')
      }
    } catch (error) {

...
>(
  (
    ...
    // Efeito para lidar com o clique no botão de menu
    React.useEffect(() => {
      const handleCollapse = (event: CustomEvent) => {
        setOpen(!event.detail.collapsed)
      }

      // Evento para toggle da sidebar em dispositivos móveis
    ...
    if (isMobile) {
      return (
        <div
          ...
          className={cn(
            "fixed inset-y-0 z-50 flex flex-col bg-sidebar text-sidebar-foreground transition-transform duration-300",

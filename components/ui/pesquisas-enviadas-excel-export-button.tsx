...
export interface PesquisasEnviadasExcelExportButtonProps {
...
export function PesquisasEnviadasExcelExportButton({
  graphData,
  lojas,
  title,
  isLoading = false,
  disabled = false,
  className
}: PesquisasEnviadasExcelExportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      excelExportService.exportPesquisasEnviadasToExcel(graphData, lojas, title)
      
      if (!graphData || graphData.length === 0) {
        toast.success('Arquivo Excel exportado! (Sem dados no gráfico)')
      } else {
        toast.success('Dados de pesquisas enviadas dos últimos 6 meses exportados com sucesso!')
      }
    } catch (error) {

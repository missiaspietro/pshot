...
export interface AniversariosDetalhadoExcelExportButtonProps {
  graphData: any[] // Dados do gráfico no formato { mes: string, [loja]: number }
...
export function AniversariosDetalhadoExcelExportButton({
  graphData,
  lojas,
  title,
  isLoading = false,
  disabled = false,
  className
}: AniversariosDetalhadoExcelExportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      excelExportService.exportAniversariosDetalhadoToExcel(graphData, lojas, title)
      
      if (!graphData || graphData.length === 0) {
        toast.success('Arquivo Excel exportado! (Sem dados no gráfico)')
      } else {
        toast.success('Dados detalhados de aniversários dos últimos 6 meses exportados com sucesso!')
      }
    } catch (error) {

...
export interface CashbackDetalhadoExcelExportButtonProps {
  graphData: any[] // Dados do gráfico no formato { mes: string, loja{numero}: number }
...
export function CashbackDetalhadoExcelExportButton({
  graphData,
  lojas,
  title,
  isLoading = false,
  disabled = false,
  className
}: CashbackDetalhadoExcelExportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      excelExportService.exportCashbackDetalhadoToExcel(graphData, lojas, title)
      
      if (!graphData || graphData.length === 0) {
        toast.success('Arquivo Excel exportado! (Sem dados no gráfico)')
      } else {
        toast.success('Dados detalhados de cashback dos últimos 6 meses exportados com sucesso!')
      }
    } catch (error) {

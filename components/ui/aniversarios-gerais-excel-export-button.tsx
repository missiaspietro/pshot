...
export interface AniversariosGeraisExcelExportButtonProps {
  graphData: any[] // Dados do gráfico no formato { name: string, valor: number }
  title: string
  isLoading?: boolean
  disabled?: boolean
  className?: string
...
export function AniversariosGeraisExcelExportButton({
  graphData,
  title,
  isLoading = false,
  disabled = false,
  className
}: AniversariosGeraisExcelExportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      excelExportService.exportAniversariosGeraisToExcel(graphData, title)
      
      if (!graphData || graphData.length === 0) {
        toast.success('Arquivo Excel exportado! (Sem dados no gráfico)')
      } else {
        toast.success('Dados de aniversários gerais dos últimos 6 meses exportados com sucesso!')
      }
    } catch (error) {

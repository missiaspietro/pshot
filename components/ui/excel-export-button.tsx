...
import { CashbackData } from '@/lib/cashback-service'
...
export interface ExcelExportButtonProps {
  graphData: any[] // Dados do gráfico no formato { loja: string, quantidade: number }
...
export function ExcelExportButton({
  ...
  const handleExport = async () => {
    if (!graphData || graphData.length === 0) {
      toast.info('Não há dados de cashback para exportar')
      return
    }
    ...
    try {
      ...
      excelExportService.exportGraphDataToExcel(graphData, currentPeriod)
      
      const periodText = currentPeriod === 1 ? 'último mês' : `últimos ${currentPeriod} meses`
      toast.success(`Dados do ${periodText} exportados com sucesso!`)
    } catch (error) {
